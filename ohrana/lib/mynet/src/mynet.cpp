...
// TCP debug 
WiFiServer _dbgServer(8989);
WiFiClient _dbgClient;
...
//    server.handleClient();
    if (!_dbgClient || !_dbgClient.connected()) {
    _dbgClient = _dbgServer.available();
    if (_dbgClient) {
      Serial.println("New debug client connected");
    }
  }
  }  
}
